...
void test_detectorimg(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen, image img)
{
    double time;
    time = what_time_is_it_now();
    ...
    while (1)
    {
        im = img; //load_image_color(input,0,0);
        ...
        //draw_detections(im, dets, nboxes, 0.5, names, alphabet, 80);
        //draw_detections(img[buff_index], det, nboxes, 0.5, names, alphabet, 80);

        printf("%s: Predicted fps %f .\n", input, 1 / (what_time_is_it_now() - time));
        ...
        {
             //save_image(im, "predictions1");
#ifdef OPENCV
             //make_window("predictions1", 512, 512, 0);
             //show_image(im, "predictions1", 1);
#endif
        }
        //free_image(im);
        free_image(sized);
